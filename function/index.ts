...
function qaPrompt(client: any, brandFile?: string | null): string {
  ...
  return [
    ...
    "",
    "This episode must read as a calm, professionally shot interview podcast — never as a trailer or a highlight reel. Its default look is a SIDE-BY-SIDE TWO-UP with the host on the left and the guest on the right, both visible at once, the way the major interview podcasts are cut.",
    "",
    ...
    "2. The episode is mostly the side-by-side two-up: host on the LEFT, guest on the RIGHT, both on screen, positions identical for the whole episode and never swapped. If most of the episode is instead a sequence of single-speaker shots, rebuild it so the two-up is the steady state and single shots are the exception. Panes must be equal width and flush, with no gap, border or black strip.",
    "3. No shot is shorter than about 5 seconds, and the episode does not flick between single shots for variety. Cuts away from the two-up are occasional and land on a meaningful moment, never mid-sentence.",
    "3b. Both people are visible essentially throughout, because the two-up is the default. No stretch longer than about 30 seconds shows only one person — where you find one, return that stretch to the side-by-side layout.",
    ...
    "4b. Each person is well framed inside their pane. If a camera is still at raw framing so the person looks small, off-centre or surrounded by empty room and furniture, set a FIXED crop of about 1.15x to 1.25x on that track which centres them with the eyeline in the upper third. One constant value per camera, held for the whole episode, never animated.",
    "5. The footage fills the entire 16:9 frame: no black bars and no letterboxing. The side-by-side layout must be KEPT — it is the intended look, not a defect. Only change a layout when one of its panes is dead or empty, since an inactive feed is what leaves a black panel; in that case switch to a layout matching the number of live speakers.",
...
function producePrompt(client: any, brandFile?: string | null): string {
  ...
  return [
    ...
    "",
    "CAMERA WORK — SIDE-BY-SIDE INTERVIEW LAYOUT (this is the most important part):",
    "- THE DEFAULT LAYOUT FOR THIS EPISODE IS A SIDE-BY-SIDE TWO-UP: the interviewer/host on the LEFT and the guest on the RIGHT, both on screen at the same time, each filling their own half of the frame. This is the layout the major interview podcasts use, and it is what this episode should look like for most of its running time.",
    "- Fix those positions for the WHOLE episode. The host is always on the left, the guest is always on the right. Never swap sides mid-episode.",
    "- If a third person is part of the conversation, use an equal-width group layout that includes them, keeping the host left-most and the same order throughout.",
    "- Hold the two-up as the steady state. Because both faces are always visible, the viewer can always see who is asking and who is answering, and no extra cutting is needed to keep it alive.",
    "- Cut to a single full-frame shot of one person only when a moment genuinely earns being closer — a key answer or a strong line — then return to the side-by-side. Use this sparingly: the two-up is the flow, single shots are the exception.",
    "- Never cut in the middle of someone's sentence or thought, and hold any shot for at least about 5 seconds. Do not flick between single shots.",
    "- Inside each pane, frame that person properly: centred in their half, head and shoulders filling it, eyeline in the upper third. Both panes must be equally sized and aligned, with no gap, border or black strip between them.",
